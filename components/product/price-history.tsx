  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div
        ...
        role="img"
        aria-label={`Price trend from ${formatINR(firstPoint.price)} to ${formatINR(current)}`}
      >
        {history.map((h, i) => (
          <div
            key={i}
            ...
            style={{
              height: `${8 + ((h.price - lowest) / range) * 100}%`,
            }}
            title={`${formatINR(h.price)} on ${new Date(
              h.recorded_at,
      ...
      <p className="text-caption text-muted-foreground">
        Based on {history.length} recorded price points.
      </p>
    </div>
  );
}
